...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($class_err) && empty($subject_err) && empty($file_err) ){
        // $_POST['role'];
          // Prepare an insert statement
            $sql = "INSERT INTO `notes_table`( `class`, `subject`, `file_dir`) VALUES (?,?,?)";
           
          if($stmt = mysqli_prepare($link, $sql)){
              // Bind variables to the prepared statement as parameters
              mysqli_stmt_bind_param($stmt, "sss",$class,$subject,$file);
              ...
              if(mysqli_stmt_execute($stmt)){
                  ...
                  window.location.href="files1.php";
                  </script>';
